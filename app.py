...
def Api_connect():
    Api_Id="AIzaSyDud4wiaqg4nEhV8tjP0-jJlIwxyVv9UCQ"
...
def tables(df_channels, df_playlists, df_videos, df_comments):

    news= channels_table(df_channels)
    if news:
        st.write(news)
    else:
        playlist_table(df_playlists)
...
if st.button("collect the Data"):
    ...
    Table=tables(df_channels, df_playlists, df_videos, df_comments)
    st.success(Table)
